...
void respawnSpectator( gentity_t *ent ) {
	...
	if ( level.time > client->respawnTime ) {
		if ( g_forcerespawn.integer > 0 && level.time - client->respawnTime > g_forcerespawn.integer * 1 ) {
...
static void player_free( gentity_t *ent ) {
	...
	ent->client->respawnTime = level.time + 100;
...
void team_wins( int team ) {
	...
	for ( i = 0; i < g_maxclients.integer; i++ ) {
		...
		e->health = cl->ps.stats[ STAT_HEALTH ] = e->client->ps.stats[STAT_MAX_HEALTH] * 3 / 2;
...
void Hook_Fire( gentity_t *ent ) {
	...
	if ( !client->hook && ( ucmd->buttons & 32 ) ) {
		if ( ent->timestamp > level.time - 4000 ) {
			return;
		}
