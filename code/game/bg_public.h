...
#define CS_MAX (XQ3E_ALLOW_FEATURES)
...
typedef struct {
  ...
  float overbounce_scale; // Amount of overbounce scale to apply. Default
                          // OVERCLIP = 1.001f
...
typedef enum {
  ...
  STAT_TIME_START,    // level.time when the start trigger was hit. NULL means
                      // timer is disabled
...
#define TEAM_LOCATION_UPDATE_TIME 100
...
void BG_AddPredictableEventToPlayerstate(entity_event_t newEvent, int eventParm,
                                         playerState_t *ps, int entityNum);
...
void BG_CleanName(const char *in, char *out, int outSize,
