...
// q3 math quirks
int phy_snapvelocity;
int phy_input_scalefix;
...
void new_GroundTrace(void) {
  qboolean rampslideMin;
  qboolean slideableRamp;
  qboolean notFlatGround;
  qboolean canRampslide;
  qboolean slick;
  qboolean knockback;
  qboolean slide;
  qboolean style1;
  ...
  if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // If there is no previous frame ground, we just landed
    rampslideMin  = (pm->ps->velocity[2] >= phy_rampslide_speedmin)  ? qtrue:qfalse;
    slideableRamp = (trace.plane.normal[2] > MIN_WALK_NORMAL)        ? qtrue:qfalse;
    notFlatGround = (trace.plane.normal[2] != 1)                     ? qtrue:qfalse;
    canRampslide  = (rampslideMin && slideableRamp && notFlatGround) ? qtrue:qfalse;
    if (phy_rampslide && canRampslide){  // Set rampsliding if enabled and conditions are met
      ...
      slick     = (pml.groundTrace.surfaceFlags & SURF_SLICK)  ? qtrue:qfalse;
      knockback = (pm->ps->pm_flags & PMF_TIME_KNOCKBACK)      ? qtrue:qfalse;
      slide     = (slick || knockback)                         ? qtrue:qfalse;
      style1    = (phy_rampslide_type == VQ1 && slide)         ? qtrue:qfalse;
      if (style1){
...
void core_Weapon( void ) {
  ...
  case WP_BFG:              addTime = 4000; break;
