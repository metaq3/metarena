...
static qboolean q3a_CheckJump(void) {
  ...
  if (pm->movetype == CPM || pm->movetype == CQ3) {
    djtimer   = pm->cmd.serverTime - pm->ps->stats[STAT_TIME_LASTJUMP];
    djtimerOn = ((djtimer <= phy_jump_timebuffer) && (djtimer > 0)) ? qtrue:qfalse; // We can dj when this is true.
...
static void q3a_AirControl(vec3_t wishdir, float wishspeed) {
  ...
  if (dot > 0) {
    k = k * phy_aircontrol_amount * dot * dot * pml.frametime;
...
void q3a_move(pmove_t* pmove) {
  ...
  else                  { pm->ps->velocity[2] = floor(pm->ps->velocity[2] + 0.5f); } // No snapzones. Always snap vertical velocity, to preserve default max jump height
}
